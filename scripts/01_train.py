...
if __name__ == '__main__':
    ...
    t0 = time.time()
    out_dir = generate_nc_files('/home/hvtran/washita_clm')
    t1 = time.time()
    print('creating forcing files time: '+str(t1-t0))
    
    # ---------------------------------------------------
    
    t0 = time.time()
    generate_nc_file_stream('/home/hvtran/washita_clm', 'washita_clm')
    t1 = time.time()
    print('creating flow and wtd files time: '+str(t1-t0))
    ...
    target_flow_input = xr.open_dataset(os.path.join(NC_DIR, 'washita_clm_flow.nc'))
    press_input = xr.open_dataset(os.path.join(NC_DIR, 'washita_clm_press.nc'))
    target_wtd_input = xr.open_dataset(os.path.join(NC_DIR, 'washita_clm_wtd.nc'))
    target_clm_input = xr.open_dataset(os.path.join(NC_DIR, 'washita_clm_clm.nc'))
    ...
    # Get SWE output
    SWE_input = target_clm_input.clm[:,10:11,:,:] # variable #10
    ...
    new_static_feature_train = new_static_feature_da[:, :40, :40, :]
    press_feature_train = press_feature_da[:, [(24 * n_days) * i for i in range(n_sample)], :40, :40, :]
    ...
    press_feature_train = np.reshape(press_feature_train, (n_sample, 1, press_feature_train.shape[2], press_feature_train.shape[3],
                                                               press_feature_train.shape[4]))
    ...
    ae_optimizer = tf.keras.optimizers.Adam(learning_rate=1e-4)
    # MSE works here best
    loss_func = tf.keras.losses.MeanSquaredError()
    ...
    save_name = '3_samples_1_weeks_3_vars_8_layers_weights'
