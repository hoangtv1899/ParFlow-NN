...
def data_generator(data_dir, feature_fn, target_fn, shuffle=True,
	...
	while True:
		for i in range(n_batches):
			...
			if (feature_norms is not None) and (target_norms is not None):
				feat_norm = feature_norms[1]
				x = (x[feat_norm!=0] - feature_norms[0][feat_norm!=0]) / feature_norms[1][feat_norm!=0]
				y = (y[feat_norm!=0] - target_norms[0][feat_norm!=0]) * target_norms[1][feat_norm!=0]
...
def data_generator_convo(data_dir, feature_fn, target_fn, shuffle=True,
    ...
    """
    # Open files
    feature_file = xr.open_dataset(data_dir + feature_fn)
    target_file = xr.open_dataset(data_dir + target_fn)
...
class DataGenerator(object):
    ...
    def __init__(self, data_dir, feature_fn, target_fn, batch_size, norm_fn=None,
        ...
        with xr.open_dataset(data_dir + feature_fn) as feature_file:
            n_samples = feature_file['features'].shape[0]
            self.feature_shape = feature_file['features'].shape[1]
        self.n_batches = int(np.floor(n_samples / batch_size))
        with xr.open_dataset(data_dir + target_fn) as target_file:
            self.target_shape = target_file['targets'].shape[1]
        if fsub is not None or fdiv is not None:
            self.feature_norms = []   # This does nothing...
            with xr.open_dataset(data_dir + norm_fn) as norm_file:
                if fsub is not None: self.feature_norms.append(norm_file[fsub][:])
                if fdiv is not None:
                    if fdiv == 'range':
                        self.feature_norms.append(norm_file['feature_maxs'][:] -
                                                 norm_file['feature_mins'][:])
                    elif fdiv == 'max_rs':  # Max range, std_by_var
                        self.feature_norms.append(np.maximum(
                            norm_file['feature_maxs'][:] - norm_file['feature_mins'][:],
                            norm_file['feature_stds_by_var']
                        ))
                    elif fdiv == 'feature_stds_eps':
                        eps = 1e-10
                        self.feature_norms.append(np.maximum(norm_file['feature_stds'][:], eps))
                    else:
                        self.feature_norms.append(norm_file[fdiv][:])
