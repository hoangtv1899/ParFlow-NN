...
def wtd_cal(yi,xi,res,dz_scale,nz,dz):
    satur_arr = res[yi,xi,:,50:]
    press_arr = res[yi,xi,:,:50]
...
class ModelDiagnostics(object):
    ...
    """
    def __init__(self, predictions, targets, target_names, nlat, nlon, tvars, is_tf=False):
        # Basic setup
        ...
        if self.is_k:
            ...
            self.sample_size,self.target_size = predictions[0].shape
            self.batch_size = nlat*nlon
            p_s = []
            for p in predictions:
                p_s.append(np.reshape(p,(nlat,nlon,int(np.floor(self.sample_size / self.batch_size)),self.target_size),'F'))
            self.p = p_s
            self.t = targets
