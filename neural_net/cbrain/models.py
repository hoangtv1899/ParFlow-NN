...
def lstm_model(feature_shape, target_shape, time_shape, hidden_layers, lr, loss,
             activation='relu', batch_norm=False, dr=None, l2=None, 
             partial_relu=False, eq=False, fsub=None, fdiv=None):
    """Creates a simple lstm neural net and compiles it
    ...
    inp = Input(shape=(time_shape,feature_shape))
    # First hidden layer
    x = LSTM(hidden_layers[0],return_sequences=True)(inp)
    #x = act_layer(activation)(x)
    if batch_norm:
        x = BatchNormalization()(x)    
    # if dr is not None:
    ...
    if len(hidden_layers) > 1:
        for h in hidden_layers[1:]:
            x = LSTM(h, kernel_regularizer=l2,return_sequences=True)(x)
            #x = act_layer(activation)(x)
            if batch_norm:
                x = BatchNormalization()(x)
            if dr is not None:
                x = Dropout(dr)(x)
    # Output layer
    x = Dense(target_shape, activation=activation, kernel_regularizer=l2)(x)
