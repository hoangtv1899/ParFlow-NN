...
class ConfigSection(object):
    ...
    Currently datetime/int/float/str are supported.
    """
    def __init__(self, config, section_proxy):
        self.config = config
        self.name = section_proxy.name
        self.d = {}  # key value dict where the value is typecast to datetime/int/float/str
    ...
    def __getattr__(self, item):
        if item not in ('config', 'name', 'd'):
            ...
            return env_var or self.d.get(item, None)
