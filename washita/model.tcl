...
if {[file exists $output_dir]} {
    puts "Output folder $output_dir already exists. Please choose another name for your run."
    exit -1
}
...
#-----------------------------------------------------------------------------
# CLM Inputs - these NEED to be copied over directly to the output_dir
#-----------------------------------------------------------------------------
file copy {*}[glob -nocomplain input/clm/drv*.dat] $output_dir
...
pfset Geom.indi_input.FileName            [pwd]/input/static/indicatorfile.pfb
...
pfset Cycle.rainrec.rec.Length            150.
...
pfset Patch.z-upper.BCPressure.rain.Value -0.1
...
pfset TopoSlopesX.FileName                            [pwd]/input/static/slopex.pfb
...
pfset TopoSlopesY.FileName                            [pwd]/input/static/slopey.pfb
...
pfset Solver.CLM.MetFilePath                          [pwd]/input/forcing
...
pfset Geom.domain.ICPressure.FileName                 [pwd]/input/static/press.init.pfb
...
# Distribute inputs
#-----------------------------------------------------------------------------
pfdist -nz 1 input/static/slopex.pfb
pfdist -nz 1 input/static/slopey.pfb
pfdist input/static/indicatorfile.pfb
pfdist input/static/press.init.pfb

# To distribute forcing files, NZ must be set to the number of timesteps
foreach forcing_file [glob input/forcing/*.pfb] {
    pfdist -nz [pfget Solver.CLM.MetFileNT] $forcing_file
}
...
#-----------------------------------------------------------------------------
# Remove CLM input file(s) that we previously copied over to $output_dir 
#-----------------------------------------------------------------------------
file delete {*}[glob -nocomplain $output_dir/drv*.dat]

#-----------------------------------------------------------------------------
# Undistribute inputs and outputs
#-----------------------------------------------------------------------------
pfundist $output_dir/$runname
pfundist input/static/slopex.pfb
pfundist input/static/slopey.pfb
pfundist input/static/indicatorfile.pfb
pfundist input/static/press.init.pfb

foreach forcing_file [glob input/forcing/*.pfb] {
    pfundist $forcing_file
}
#-----------------------------------------------------------------------------
